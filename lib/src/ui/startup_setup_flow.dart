...
final class _StartupSetupFlowState extends State<StartupSetupFlow> {
  ...
  Widget _welcome() => const _SetupSection(
        ...
        body:
            'CyberForge keeps orchestration local-first. This guided setup stores secrets in a restricted local secure config store, configures optional cloud critics, and prepares an isolated local inference worker. Keys are never passed on process command lines.',
  ...
  Future<void> _next() async {
    ...
    try {
      ...
      // Same value used for every-boot vault unlock when device key is offline.
      await _store.writeSecret('vault_password', _recovery.text);
